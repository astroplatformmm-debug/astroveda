  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          className={`${cls} ${star <= rating ? "text-[#F59E0B]" : "text-[#E2E8F0]"}`}
          fill="currentColor"
          viewBox="0 0 24 24"
        >
  ...
  if (imageUrl && !imgFailed) {
    return (
      <img
        src={imageUrl}
        alt={name}
        className="w-11 h-11 rounded-full object-cover border-2 border-[#F97316]/30 flex-shrink-0"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/reviews", {
        ...
        body: JSON.stringify({ name: name.trim(), email: email.trim() || undefined, rating, message: message.trim(), profile_image: profileImage || undefined }),
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        ...
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Priya Sharma" maxLength={100}
        ...
        <div className="flex gap-1.5 items-center">
          {[1, 2, 3, 4, 5].map((star) => (
            <button key={star} type="button" onClick={() => setRating(star)} onMouseEnter={() => setHoverRating(star)} onMouseLeave={() => setHoverRating(0)}
              className="focus:outline-none transition-transform hover:scale-110" aria-label={`Rate ${star} star${star > 1 ? "s" : ""}`}>
              <svg className={`w-8 h-8 transition-colors ${star <= (hoverRating || rating) ? "text-[#F59E0B]" : "text-[#E2E8F0]"}`} fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
              </svg>
            </button>
          ))}
      ...
      <div>
        ...
        <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Share your experience with OMKKAAR Astroworld..." rows={4} maxLength={1000}
        ...
        <div className="flex items-center gap-4">
          {previewUrl ? (
            <div className="relative">
              ...
              <button type="button" onClick={() => { setProfileImage(null); setPreviewUrl(null); if (fileRef.current) fileRef.current.value = ""; }}
                className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center text-xs">x</button>
            </div>
          ) : (
  ...
  return (
    <div className="bg-[#FAF7F2] min-h-screen pb-28 lg:pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 items-start">
          ...
          <div className="relative">
            <div className="relative rounded-2xl overflow-hidden bg-gray-100 shadow-xl border border-[#E2E8F0]">
              ...
              {allImages.length > 1 && (
                <>
                  <button type="button" onClick={() => setCurrentImageIndex((i) => (i === 0 ? allImages.length - 1 : i - 1))}
                    className="absolute left-2 top-1/2 -translate-y-1/2 bg-white rounded-full p-2 shadow-md text-[#0F172A] hover:text-[#F97316] transition-colors" aria-label="Previous image">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                  </button>
                  <button type="button" onClick={() => setCurrentImageIndex((i) => (i === allImages.length - 1 ? 0 : i + 1))}
                    className="absolute right-2 top-1/2 -translate-y-1/2 bg-white rounded-full p-2 shadow-md text-[#0F172A] hover:text-[#F97316] transition-colors" aria-label="Next image">
            ...
            {allImages.length > 1 && (
              <>
                ...
                <div className="flex gap-2 mt-3 flex-wrap justify-center">
                  {allImages.map((src, i) => (
                    <button key={src + i} type="button" onClick={() => setCurrentImageIndex(i)}
          ...
          <div>
            ...
            <p className="text-3xl font-bold text-[#F97316] mt-3">&#8377;{displayPrice.toLocaleString("en-IN")}</p>
            ...
            {gemstone.options && gemstone.options.length > 0 && (
              <div className="mt-4">
                ...
                <select id="product-option" value={selectedOption ? selectedOption.label : ""}
                  onChange={(e) => { const found = (gemstone.options || []).find((o) => o.label === e.target.value); setSelectedOption(found ?? null); }}
            ...
            {gemstone.ringMaterialEnabled && gemstone.ringMaterials && gemstone.ringMaterials.length > 0 && (
              <div className="mt-4">
                ...
                <select id="ring-material" value={selectedRingMaterial ? selectedRingMaterial.label : ""}
                  onChange={(e) => { const found = (gemstone.ringMaterials || []).find((m) => m.label === e.target.value); setSelectedRingMaterial(found ?? null); }}
                  className="mt-1 w-full border border-[#E2E8F0] rounded-lg px-3 py-2 bg-white focus:outline-none focus:border-[#F97316]">
                  <option value="">No ring setting</option>
                  {gemstone.ringMaterials.map((mat) => (
                    <option key={mat.label} value={mat.label}>{mat.label} {mat.extraPrice > 0 ? `(+&#8377;${mat.extraPrice})` : "(included)"}</option>
                  ))}
                </select>
                {selectedRingMaterial && selectedRingMaterial.extraPrice > 0 && (
                  <p className="mt-1 text-xs text-[#64748B]">Ring setting adds &#8377;{selectedRingMaterial.extraPrice} to the price</p>
                )}
              </div>
            )}
            ...
            <div className="mt-6 flex flex-col gap-3">
              <Link href={checkoutHref}
                className="block w-full bg-[#F97316] text-white py-4 rounded-xl font-semibold text-lg hover:bg-[#EA6C0A] transition-all duration-200 text-center shadow-md hover:shadow-lg">
                Buy Now
              </Link>
              ...
              <Link href="/shop" className="block text-center text-[#64748B] hover:text-[#F97316] text-sm">
                &#8592; Back to Shop
              </Link>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-[#E2E8F0] grid grid-cols-2 gap-4">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full bg-[#FFF7ED] flex items-center justify-center text-[#F97316] flex-shrink-0">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                </div>
                <div><h4 className="font-bold text-sm text-[#0F172A]">Secure Payment</h4><p className="text-xs text-[#64748B]">256-bit SSL encryption</p></div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div><h4 className="font-bold text-sm text-[#0F172A]">Fast Shipping</h4><p className="text-xs text-[#64748B]">Insured, ships in 24h</p></div>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full bg-green-50 flex items-center justify-center text-green-600 flex-shrink-0">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                </div>
                <div><h4 className="font-bold text-sm text-[#0F172A]">Easy Returns</h4><p className="text-xs text-[#64748B]">48-hour return window</p></div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div><h4 className="font-bold text-sm text-[#0F172A]">Lab Certified</h4><p className="text-xs text-[#64748B]">AAA++ authentic gems</p></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 bg-white border-t border-[#E2E8F0] p-3 flex gap-2 lg:hidden shadow-2xl">
        <a href={whatsappUrl} target="_blank" rel="noopener noreferrer"
          className="flex-1 flex items-center justify-center gap-1.5 border-2 border-green-500 text-green-700 py-3 rounded-xl font-semibold text-sm">
          <svg className="w-4 h-4" viewBox="0 0 24 24" fill="#16a34a"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.126.554 4.124 1.523 5.86L.057 23.885a.5.5 0 00.611.611l6.115-1.526A11.945 11.945 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.815 9.815 0 01-4.992-1.364l-.358-.213-3.712.926.943-3.623-.234-.372A9.818 9.818 0 012.182 12C2.182 6.58 6.58 2.182 12 2.182S21.818 6.58 21.818 12 17.42 21.818 12 21.818z"/></svg>
          WhatsApp
        </a>
        <Link href={checkoutHref}
